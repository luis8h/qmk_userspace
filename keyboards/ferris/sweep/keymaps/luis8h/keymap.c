...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        // this is basically a custom override which fixes the ctrl-right override not working for LGUI(KC_RIGHT) keycodes because the override is only for KC_RIGHT
        case DUAL_FUNC_LGUIRIGHT:
            if (record->tap.count > 0) {
                if (record->event.pressed) {
                    if ((mod_state & MOD_MASK_CTRL) && is_macos) {
